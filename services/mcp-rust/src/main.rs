...
impl Mcp for McpService {
    type GetVulnerabilitiesStream = Pin<Box<dyn Stream<Item = Result<Vulnerability, Status>> + Send>>;
    ...
    async fn analyze_code(
        ...
        let mut ai_lock = self.ai_client.lock().await;
            ...
            match client.run_static_analysis(
                ...
                Ok(static_findings) => {
                    ...
                    match client.extract_ast(
                        ...
                        Ok(ast_data) => {
                            tracing::debug!("AST extraction complete: {} functions", ast_data.functions);
    ...
    async fn get_vulnerabilities(
    ...
    ) -> Result<Response<Self::GetVulnerabilitiesStream>, Status> {
        ...
        let stream: Self::GetVulnerabilitiesStream = Box::pin(stream);
