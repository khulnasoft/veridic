...
class AiServiceImpl(ai_service_pb2_grpc.AiServiceServicer):
    ...
    async def Reason(self, request, context):
        ...
        try:
            # Parse findings for correlation
            # In production, would deserialize from JSON
            insights = await ai_reasoner.correlate_findings(
                findings=[],  # TODO: deserialize from request.findings
                code=request.code_snippet,
                language="unknown",  # TODO: add language to request
            )
    ...
    async def GenerateReport(self, request, context):
        ...
        try:
            # In production: deserialize findings from JSON
            findings = []
            metadata = {
                "language": "unknown",
